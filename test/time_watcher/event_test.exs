defmodule TimeWatcher.EventTest do
  ...
  describe "JSON round-trip" do
    ...
    test "from_json/1 returns error for invalid event_type" do
      json = Jason.encode!(%{timestamp: 123, repo: "r", path_id: "abc", event_type: "bad"})
      assert {:error, :invalid_event_type} = Event.from_json(json)
    end

    test "from_json/1 reads legacy hashed_path key" do
      json =
        Jason.encode!(%{
          timestamp: 123,
          repo: "r",
      ...
      {:ok, event} = Event.from_json(json)
      assert event.path_id == "abc123"
    end
  end
end
